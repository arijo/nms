...
require([
	...
	'underscore', 
	'backbonejs',
	'shared/nms/utils/sniffer'

], function( require, $, _, Backbone, Sniffer ){

	// BOOTSTRAP THE NMS APPLICATION
	var settings = NMS.SETTINGS;

	// DETECT OS, BROWSER AND VERSION
	Sniffer.init();
	settings.OS = Sniffer.OS;
	settings.browser = Sniffer.browser;
	settings.version = Sniffer.version;
	...
	if( settings.desktop) {
		...
		NMS.APP = require(['desktop/nms/nms'], function( NmsApp) {
	...
	// BOOTSTRAP THE NMS APPLICATION ( TABLET VERSION)
	if( settings.tablet) {

		NMS.APP = require(['tablet/nms/nms'], function( NmsApp) {
